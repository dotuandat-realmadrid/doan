...
// Hàm kiểm tra trạng thái yêu thích của nhiều sản phẩm
function checkWishlistStatus(userId, productIds, callback) {
    if (!userId || productIds.length === 0) {
        console.log('No userId or productIds, returning empty status');
        callback({});
        return;
    }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.code === 1000) {
                    wishlistStatus[productId] = response.result;
                }
                completedRequests++;
                if (completedRequests === productIds.length) {
                    console.log('Final wishlist status:', wishlistStatus);
                    callback(wishlistStatus);
                }
            },
            error: function(xhr, status, error) {
                console.error(`Error checking wishlist status for product ${productId}: ${status} - ${error} - ${xhr.responseText}`);
                if (xhr.status === 404) {
                    wishlistStatus[productId] = false; // Giả sử sản phẩm không có trong wishlist nếu 404
                }
                completedRequests++;
                if (completedRequests === productIds.length) {
                    console.log('Final wishlist status:', wishlistStatus);
...
function loadProductByCategory(categoryCode, tabId) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            // Lấy danh sách productIds để kiểm tra trạng thái yêu thích
            let productIds = products.map(product => product.id);
            checkWishlistStatus(userId, productIds, function(wishlistStatus) {
                ...
                products.forEach((product, index) => {
                    ...
                    const isFavorited = wishlistStatus[product.id] || false;
