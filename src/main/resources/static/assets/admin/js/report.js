$(document).ready(function() {
	...
	function formatCurrency(amount) {
		return (amount / 1000000).toFixed(2) + ' triệu VNĐ';
	}
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (response.code === 1000 && response.result) {
					...
					$('#total-revenue').text(`${data.totalRevenue.toLocaleString('vi-VN')}đ` || 0);
					$('#total-purchase').text(`${data.totalInventoryPrice.toLocaleString('vi-VN')}đ` || 0);
					const profit = (data.totalRevenue || 0) - (data.totalInventoryPrice || 0);
					if (profit < 0) {
						$('#profit').addClass('text-danger').text(`↓ -${Math.abs(profit).toLocaleString('vi-VN')}đ`);
					} else {
						$('#profit').addClass('text-success').text(`↑ ${profit.toLocaleString('vi-VN')}đ`);
					}
				} else {
