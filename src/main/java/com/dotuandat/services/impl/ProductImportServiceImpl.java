...
public class ProductImportServiceImpl implements ProductImportService {
     ...
     * Tạo prompt chi tiết - yêu cầu AI sinh tất cả thông tin
     */
    private String createDetailedPrompt(
            List<String> validCategoryCodes, List<String> validSupplierCodes, int quantity) {
        return String.format(
                ...
                quantity, String.join(", ", validCategoryCodes), String.join(", ", validSupplierCodes), quantity);
    }
    ...
    private void validateAIGeneratedData(
            List<ProductCreateRequest> requests, List<String> validCategoryCodes, List<String> validSupplierCodes) {
        ...
        for (int i = 0; i < requests.size(); i++) {
            ...
            if (request.getSupplierCode() == null
                    || request.getSupplierCode().trim().isEmpty()) {
